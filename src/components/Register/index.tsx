...
function Register({ onSubmit }: { onSubmit: (data: object) => void }) {
  ...
  const handleSubmitForm = ({ rePassword, ...data }: { rePassword: string }) => onSubmit(data);

  const { userName, email, password, rePassword } = errors;
  const styleInput = 'w-full rounded-md border border-gray-300 px-3 py-2 placeholder-gray-300 focus:outline-none';
            ...
            <form className="mb-4" onSubmit={handleSubmit(data => handleSubmitForm(data))}>
              <div className="mb-4">
                ...
                <input
                  ...
                  className={clx(styleInput, { 'border-red-500': userName })}
                  {...register('userName')}
                />
                {userName ? <span className="text-red-500">{userName?.message}</span> : ''}
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  id="email"
                  placeholder="Your email address"
                  className={clx(styleInput, { 'border-red-500': email })}
                  {...register('email')}
                />
                {email ? <span className="text-red-500">{email?.message}</span> : ''}
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  id="password"
                  placeholder="Your email address"
                  className={clx(styleInput, { 'border-red-500': password })}
                  {...register('password')}
                />
              ...
              <div className="mb-6">
                <div className="mb-1 flex justify-between">
                  <label htmlFor="rePassword" className="text-sm text-gray-600">
                    Re-Password
                  </label>
                </div>
                <input
                  ...
                  {...register('rePassword')}
                />
                {rePassword ? <span className="text-red-500">{rePassword?.message}</span> : ''}
              </div>
              <div className="mb-6">
                <button
                  type="submit"
